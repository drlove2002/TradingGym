...
class StocksEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Dict({
            ...
            'features': spaces.Box(low=0, high=INF, shape=self.shape, dtype=np.float64),
        })
        ...
        self._portfolio_values = deque([self._balance + self._equity] * window_size, maxlen=window_size)
    ...
    def _features(self):
        ...
        return self.df[self._current_tick - self.window_size: self._current_tick].copy()
    ...
    def _sharpe_ratio(self):
        ...
        rolling_returns = np.diff(self._portfolio_values) / np.array(self._portfolio_values)[:-1]
    ...
    def _get_reward(self, action: Action, cost: float, fee: float) -> float:
        ...
        # Calculate the return since the last tick
        last_value = self._portfolio_values[-1]
        if last_value == 0:
            last_value = current_value
        return_since_last_tick = (current_value - last_value) / last_value
        ...
        reward += return_since_last_tick - (0.01 * (cost + fee)) - (0.1 * (sharpe_ratio-self._last_sharpen_ratio))
    ...
    def step(self, action):
        ...
        if (self._balance <= 0 and self._equity <= 0) or (self._current_tick >= self._end_tick):
            self._done = True
    ...
    def render(self, mode='human'):
        ...
        fig, (ax1, ax2, ax3, ax4) = plt.subplots(
            nrows=4, ncols=1, figsize=(10, 10), facecolor='white',
            sharex=True, gridspec_kw={"height_ratios": [3, 1, 1, 1]}  # type: ignore
        )
        fig: plt.Figure
        ...
        for index, row in df_chunk.iterrows():
            ...
            ax1.plot([index, index], [row['Low'], row['High']], color=color, linewidth=((5 * 10) / self.window_size))
            ax1.plot([index, index], [row['Open'], row['Close']], color=color, linewidth=((25 * 10) / self.window_size))

        # Plot the EMA and SMA line and color it
        ax1.plot(df_chunk.index, df_chunk['EMA'], color='blue', label='EMA', linewidth=1.3)
        ax1.plot(df_chunk.index, df_chunk['SMA'], color='yellow', label='SMA', linewidth=1.2)
        ax1.legend()
        ...
        ax1.scatter(self.df.index[self._current_tick], current_position, marker='*', s=200, color='purple', zorder=10)

        plt.xticks(df_chunk.index, pd.to_datetime(df_chunk.index).strftime("%Y-%m-%d"), rotation=45)
        fig.tight_layout()
        plt.show()
    ...
    def legal_actions(self):
        ...
        actions = []
        if self._balance > 0:
            actions.append((Action.BUY, min(self.max_shares, self._balance // self._current_price)))  # Buy action
        if self._qtn > 0:
            actions.append((Action.SELL, min(self.max_shares, self._qtn)))  # Sell action
